module Fetch_ctrl(
    ...
    input        redirect_valid_i, input [31:0] redirect_pc_i,
    input        invalidate_i, output       invalid_complete,
    input        ibuf_ready_i, input        ibuf_almost_full_i, 
    output       ibuf_push_o, output       ibuf_flush_o,
    input        ibus_hit_i, input        ibus_ready_i, input        inst_valid_i, 
    output       ibus_fetch_vld_o, input        flush_status_i,
    output       ibus_flush_o, output       ibus_invld_o, output [31:0] ibus_addr_o
);
    reg [31:0] pc_reg;
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        else if (pc_en) pc_reg <= redirect_valid_i ? redirect_pc_i : (pc_reg + 32'd4);
    end
